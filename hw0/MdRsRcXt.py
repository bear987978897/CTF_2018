import hashlib
import sys
import binascii
import struct

if len(sys.argv) != 2:
    print('Usage: python3 task.py <password>')
    exit(255)

# MD5 START
def md5(s):
    return hashlib.md5(s.encode('ascii')).hexdigest()

hashs = ''.join(map(md5, sys.argv[1]))
m = int(hashs, 16)

# RSA START
a = int('''

'''.replace('\n', ''), 16)

b = int('''

'''.replace('\n', ''), 16) * a

assert( m < b )
def rsa(m):
    m = pow(m, 65537, b)
    return m

def egcd(a, b):
    if a == 0:
        return (b, 0, 1)
    else:
        g, y, x = egcd(b % a, a)
        return (g, x - (b // a) * y, y)

def modinv(a, m):
    g, x, y = egcd(a, m)
    if g != 1:
        raise Exception('modular inverse does not exist')
    else:
        return x % m
d = modinv(65537, (a - 1) * (b // a - 1))

def decode_rsa(m):
    m = pow(m, d, b)
    return m

# ascii to byte list[512]
def int_to_byte_list(m):
    m = hex(m)[2:].rjust(1024, '0')
    m = binascii.a2b_hex(m)
    m = list(m)
    return m
def byte_list_to_int(m):
    def format_x(m):
        return(format(m, 'x').rjust(2, '0'))
    hex_list = ''.join(map(format_x, m))
    return int(hex_list, 16)

K = hex(a).encode('ascii')
# generate S: byte[256] 
# RC4 start
def rc4(m):
    K = hex(a).encode('ascii')
    S = list(range(256))
    j = 0
    for i in range(256):
        j = (j + S[i] + K[i % len(K)]) % 256
        S[i], S[j] = S[j], S[i]
    
    j = 0
    for k in range(len(m)):
        i = (k + 1) % 256
        j = (j + S[i]) % 256
        S[i], S[j] = S[j], S[i]
        m[k] ^= S[(S[i] + S[j]) % 256]
    return m

# XTEA
def xtea(m):
    k = struct.unpack("<4L", K[:16])
    for i in range(0, len(m), 8):
        v0, v1 = struct.unpack("<2L", bytes(m[i:i+8]))
        sum, delta, mask = 0, 0x9e3779b9, 0xffffffff
        for round in range(32):
            v0 = (v0 + (((v1 << 4 ^ v1 >> 5) + v1) ^ (sum + k[sum & 3]))) & mask
            sum = (sum + delta) & mask
            v1 = (v1 + (((v0 << 4 ^ v0 >> 5) + v0) ^ (sum + k[sum >> 11 & 3]))) & mask
        e = struct.pack("<2L", v0, v1)
        for j in range(8):
            m[i+j] = e[j]
    return m
def decode_xtea(m):
    k = struct.unpack("<4L", K[:16])
    for i in range(0, len(m), 8):
        v0, v1 = struct.unpack("<2L", bytes(m[i:i+8]))
        delta, mask = 0x9e3779b9, 0xffffffff
        sum = delta * 32
        for round in range(32):
            v1 = (v1 - (((v0 << 4 ^ v0 >> 5) + v0) ^ (sum + k[sum >> 11 & 3]))) & mask
            sum = (sum - delta) & mask
            v0 = (v0 - (((v1 << 4 ^ v1 >> 5) + v1) ^ (sum + k[sum & 3]))) & mask
        e = struct.pack("<2L", v0, v1)
        for j in range(8):
            m[i+j] = e[j]
    return m 
m = rsa(m)
m = int_to_byte_list(m)
m = rc4(m)
m = xtea(m)

m = bytes(m)
m = binascii.b2a_hex(m)

A = b'''

'''.replace(b'\n', b'')

flag = int_to_byte_list(int(A, 16))
flag = decode_xtea(flag)
flag = rc4(flag)
flag = byte_list_to_int(flag)
flag = decode_rsa(flag)
flag = format(flag, 'x')
dictionary = [md5(chr(int(x))) for x in range(128)]
i = 0
ascii_list = []
while (i < len(flag)):
    ch = flag[i: i + 32]
    i += 32
    ascii_list.append(format(dictionary.index(ch), 'x').rjust(2, '0'))
flag = ''.join(ascii_list)
print(binascii.unhexlify(flag))


if m == A:
    print('OK')
else:
    print('Nope')
